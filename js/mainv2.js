...
$("#sendform").click(function(e){
	...
	if($("#hvFormicpna").valid()===true){
		$.ajax({ url: 'process2.php', type: 'POST', dataType: 'json', data: fdata })
                .done(function(data) {
                    if(data.rpta==='ok') {
						//sendblue
						var attributes = {};
					 ...
					 trackEventemBlue(email,eventName,attributes,apikey);
                       ...
                       /* dataLayer.push({
                        ...
                        window.location.href="/gracias.php?p=3";
    ...
    $('form').on('submit', function(e){
	   ...
	   if (!$("#pauta").is(":checked")) {
        ...
        if(error==0&&blockForm){
            blockForm=false;
            $.ajax({ url: '/v2/process.php', type: 'POST', dataType: 'json', data: f.serializeArray() })
                .done(function(data) {
...
$(window).load(function(){
	'use strict';
	$('#programa').val('Para Jóvenes y Adultos').trigger('change');
	
	setTimeout(function(){
		$('#sedeciudad').val('Miraflores').trigger('change');
	},800);
	
	setTimeout(function(){
		$('#nivel').val('Básico').trigger('change');
	},1000);
	
	setTimeout(function(){
		$('#curso').val('Interdiario-Nocturno L-M-V').trigger('change');
	},1500);
	
	setTimeout(function(){
		$("#mostrar_informacion").trigger('click');
		
	},1800);
	
	setTimeout(function(){
		$("#sedes_f").val('Miraflores').trigger('change');
	},500);
